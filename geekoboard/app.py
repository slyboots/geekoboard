...
def current_hour():
    ...
    current = datetime.datetime.now(datetime.timezone(-datetime.timedelta(hours=6))).hour
    if DEBUG:
        print(f"lambda.current_hour: {current}")
    return current
...
def get_schedule_timeline(sheets=None):
    ...
    if not values:
        ...
        if DEBUG:
            print(f"lambda.get_schedule_timeline: {json.dumps(timeline_formatted)}")
...
def get_agent_schedules(sheets=None):
    ...
    if not values:
        ...
        if DEBUG:
            print(f"lambda.get_agent_schedules(): {json.dumps(values)}")
        return values
def current_agent_statuses():
    ...
    for row in schedules:
        ...
        schedule = row[1:]
        if DEBUG:
            print(f"lambda.current_agent_statuses: block={timeblock} {agent} schedule: {json.dumps(schedule)}")
        try:
            agent_statuses[agent] = schedule[timeblock]
        except IndexError:
            agent_statuses[agent] = ""
    if DEBUG:
        print('Agent statuses: %s', json.dumps(agent_statuses))
